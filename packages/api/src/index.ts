...
app.post("/api/pause", authMiddleware, async (req: Request, res: Response) => {
  try {
    const orgId = getOrgId(req);
...
app.get("/api/focus", authMiddleware, async (req: Request, res: Response) => {
  try {
    const orgId = getOrgId(req);
    const redis = await getRedisClient();
    ...
    for (const id of focusedIds) {
      const textResult = await redis.hget(
        tenantKeys.cache.postText(orgId, id),
        "text",
      );
      const text: string = typeof textResult === "string" ? textResult : "";
      const classificationsResult = await redis.hget(
        tenantKeys.focus.meta(orgId, id),
        "targetClassifications",
      );
      const classificationsRaw: string | null =
        typeof classificationsResult === "string"
  ...
  try {
    ...
    if (!urls || !Array.isArray(urls) || urls.length === 0) {
      return res.status(400).json({
        success: false,
        error: "Provide urls array, remove id, or clear: true",
      });
    }
...
app.patch(
  "/api/focus/:postId/classifications",
  authMiddleware,
  async (req: Request, res: Response) => {
    try {
      const orgId = getOrgId(req);
      ...
      if (
        !Array.isArray(targetClassifications) ||
        !targetClassifications.every((c: string) =>
          validClassifications.includes(c),
        )
      ) {
        return res.status(400).json({
          success: false,
          error:
            "Invalid classifications. Must be array of: hostile, friendly, neutral",
        });
      }

      // Check if post is focused
      const isFocused = await redis.sismember(
        tenantKeys.focus.active(orgId),
        postId,
      );
      if (!isFocused) {
        return res
          .status(404)
          .json({ success: false, error: "Post not found in focused posts" });
      }

      // Store classifications
      await redis.hset(tenantKeys.focus.meta(orgId, postId), {
        targetClassifications: JSON.stringify(targetClassifications),
      });
...
app.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      // Get Redis stats
      const paused =
        (await redis.get(tenantKeys.control.paused(orgId))) === "true";
