...
router.get("/threads/callback", async (req: Request, res: Response) => {
  try {
    ...
    if (error) {
      ...
      return res.redirect(
        `/?error=${encodeURIComponent(String(error_description))}`,
      );
    }
    ...
    // Encrypt token using shared GCM encryption (fail-closed if key missing)
    ...
    console.log(
      `[OAuth] Successfully connected Threads account for org ${orgId}`,
    );
